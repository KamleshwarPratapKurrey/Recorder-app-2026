...
export default function HomeScreen() {
  ...
  // Load recordings and handle keyboard listeners
  useEffect(() => {
    ...
    // Listeners to pull system keyboard dimensions dynamically
    const showEvent =
  ...
  // Handle clean scrolling adjustment when focusing an input box
  const handleInputFocus = (index: number) => {
    // Delays scrolling slightly to allow the list container padding to update first
    setTimeout(() => {
      if (flatListRef.current) {
        flatListRef.current.scrollToIndex({
          index,
          viewPosition: 0.3, // Centers the element neatly above the keyboard view
          animated: true,
        });
      }
    }, 150);
  ...
  async function deleteSelectedRecordings() {
    ...
    Alert.alert(
      "Delete Selected",
      `Are you sure you want to delete the ${selectedIds.length} selected recordings permanently?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Delete All",
  ...
  const renderRecordingCard = ({
    ...
    const timeCompleted = isCurrentPlaying ? playbackPosition : 0;
    const timeRemaining = isCurrentPlaying
    ...
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      ...
      {filteredRecordings.length === 0 ? (
        ...
        <FlatList
          ...
          // Dynamically expands list padding on keyboard show so hidden cards can be scrolled upward
          contentContainerStyle={[
            styles.listScrollContent,
            { paddingBottom: keyboardHeight > 0 ? keyboardHeight + 100 : 160 },
          ]}
...
const styles = StyleSheet.create({
  ...
  cardCheckboxHitbox: { paddingVertical: 10, paddingRight: 12 },
  cardCheckboxOutline: {
    width: 22,
    height: 22,
    borderRadius: 11,
  ...
  durationText: { fontSize: 14, color: "#6C757D", fontWeight: "500" },
  timelineContainer: { marginBottom: 12, paddingHorizontal: 4 },
  ...
  actionButtonText: { color: "#000000", fontSize: 13, fontWeight: "600" },
